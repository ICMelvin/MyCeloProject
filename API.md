...
**Note:** Either `contractPath` (local file) or `sourceCode` (string) must be provided.
...
const initialRes = await fetch('http://localhost:3000/scan', {
  ...
  body: JSON.stringify({ contractPath: './contract.sol' }),
});
...
curl -X POST http://localhost:3000/scan \
  ...
  -d '{"contractPath": "./test-contracts/VulnerableVault.sol"}'
...
|--------|-------|-------------|
| 400 | Bad Request | Missing contractPath or sourceCode |
